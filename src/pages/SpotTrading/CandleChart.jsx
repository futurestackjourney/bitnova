...
export default function CandleChart({
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      {/* Header / state */}
      <div className="absolute right-2 top-2 z-10 px-2 py-1 rounded bg-black/50 text-xs text-gray-300">
        {symbol} • {interval} • {data.length} candles {loading ? "• Loading…" : ""}
        {errMsg ? <span className="text-red-400 ml-2">({errMsg})</span> : null}
      </div>

      {/* Canvas chart */}
      <canvas ref={canvasRef} className="block w-full h-full p-3 rounded-lg bg-zinc-100 dark:bg-card" />

      {/* Empty state */}
      {!loading && data.length === 0 && (
        <div className="absolute inset-0 grid place-items-center text-zinc-600 text-sm">
          No data
        </div>
      )}
    </div>
